...
function renderTopics() {
  ...
  els.topicsVibe.textContent = `${theme.topics.length} topics, 100 questions each — or Random Mix to shuffle the whole theme.`;
  ...
  els.topicGrid.innerHTML = topics.map((topic) => {
    ...
    return `
      <button class="topic-card${random ? " random" : ""}" data-topic="${topic.key}" type="button">
        ...
        <span class="topic-card-meta">${random ? "Shuffled mix →" : "100 questions · 5 / 10 / 20 →"}</span>
      </button>
    `;
  }).join("");
...
function renderLevels() {
  ...
  els.levelsVibe.textContent = topic.random
    ...
    : `A 100-question bank, all about ${topic.name}. Beginner, Intermediate, and Advanced draw 5, 10, or 20 clues onto one crossword.`;
